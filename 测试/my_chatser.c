...
sign_up(void *arg)
{
    ...
    sprintf(buf,"{\"sign_up\":\"%s\"}",acountnumber);
    ...
    printf("%s\n",answer);
    sem_wait(&sem);
    sprintf(cmd,"insert user_data values(\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",0,-1)",acountnumber,name,password,question,answer);
    while(mysql_query(&mysql,cmd))
        ;
    sem_post(&sem);
    sprintf(cmd,"用户%ld注册成功",acount_number-1);
    write_log(cmd);
    return NULL;
}
...
deliver_file(void *arg)
{
    ...
    sprintf(buf,"{\"deliver_file\":{\"way\":\"begin\",\"path\":\"%s\",\"recv_account\":\"%s\"}}",path,recv_account);
    send(socket,buf,1024,0);
    while(recv(temp.connfd,buf,1024,0))
    {
       send(socket,buf,1024,0);
    ...
    sprintf(buf,"{\"deliver_file\":{\"way\":\"end\",\"path\":\"%s\",\"recv_account\":\"%s\"}}",path,recv_account);
...
create_group(void *arg)
{
    ...
    sprintf(group_account,"%d",account_group);
    account_group++;
...
scan_pri_message(void*)
{
    ...
    next = cJSON_GetObjectItem(item,"account");
    sprintf(account,"%s",next->valuestring);
    sprintf(cmd, "select *from chat_messages where send_user = \"%s\" or recv_user = \"%s\"",account,account);
    sem_wait(&sem);
    mysql_query(&mysql,cmd);
    res = mysql_store_result(&mysql);
